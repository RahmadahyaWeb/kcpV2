    ...
    public function potong_piutang()
    {
        ...
        if ($jumlah_details == $pass) {
            try {
                ...
                $cek_no_piutang_kcpinformation = $kcpinformation->table('trns_pembayaran_piutang_header')
                    ->where('nopiutang', $this->customer_payment_header->no_piutang)
                    ->first();

                if (!$cek_no_piutang_kcpinformation) {
                    $kcpinformation->table('trns_pembayaran_piutang_header')
                        ->insert([
                            ...
                            'modi_date'         => $this->customer_payment_header->crea_date,
                        ]);
                }
