<ul class="menu-inner py-1">
    @foreach ($menus as $menu)
        ...
        @else
            <li class="menu-item {{ $menu['active'] ? 'active' : '' }}" x-data="{ isOpen: {{ $menu['active'] ? 'true' : 'false' }} }"
                :class="{ 'open': isOpen }">
                    ...
                    <ul class="menu-sub" x-show="isOpen" x-collapse>
                        @foreach ($menu['children'] as $child)
                            <li class="menu-item {{ $child['active'] ? 'active' : '' }}">
                                <a href="{{ route($child['route']) }}" class="menu-link" wire:navigate @click="isMenuExpanded = !isMenuExpanded">
